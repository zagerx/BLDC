...
void update_feedback(struct device *dev, float dt)
{
	...
	// 计算机械角度
	float current_mech_angle = (two_pi / cpr_f) * data->total_counts * (float)cfg->params->direction;
	...
	if (data->accumulated_dt > 0.0002f) // 或 0.002f
	{
		...
		data->mech_vel = (1.0f - filter_alpha) * data->mech_vel + filter_alpha * raw_vel * K_LINE;
	}
