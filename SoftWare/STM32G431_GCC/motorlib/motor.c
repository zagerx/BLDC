...
void debug_update_foc_data(float *input, enum foc_parameters_index flag)
{
	...
	{
		write_foc_param_(foc_param, INDEX_POS_PI, input);
		TRAN_STATE(m_data->state_machine, motor_debug_idle_state);
	}
	else if (flag == INDEX_POSITION_TAR)
	{
		write_foc_param_(foc_param, INDEX_POSITION_TAR, input);
		s_planner_update_target((m_data->s_planner), input[0]);
	}
	else if (flag == INDEX_VP_PI)
	{
		write_foc_param_(foc_param, INDEX_VP_PI, input);
		TRAN_STATE(m_data->state_machine, motor_debug_idle_state);
	}
}
