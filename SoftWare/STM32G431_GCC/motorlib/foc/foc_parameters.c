...
void write_foc_param_(struct foc_parameters *foc_param, enum foc_parameters_index flag, float *data)
{
	...
	{ // PI参数更新
		...
		kp = data[0];
		ki = data[1];
		foc_param->pos_pi_control.kp = kp;
		foc_param->pos_pi_control.ki = ki;
	}
...
void read_foc_param_(struct foc_parameters *foc_param, enum foc_parameters_index flag, float *data)
{
	...
	{ // PI参数更新
		float kp, ki;
		kp = foc_param->pos_pi_control.kp;
		ki = foc_param->pos_pi_control.ki;
		data[0] = kp;
		data[1] = ki;
	}
	else if (flag == INDEX_VP_PI)
	{
		data[0] = foc_param->velocity_pi_control.kp;
		data[1] = foc_param->velocity_pi_control.ki;
		data[2] = foc_param->pos_pi_control.kp;
		data[3] = foc_param->pos_pi_control.ki;
	}
}