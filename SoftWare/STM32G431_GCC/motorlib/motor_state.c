...
fsm_rt_t motor_idle_state(fsm_cb_t *obj)
{
	...
	{
	case ENTER:
		s_planner_init(m_data->s_planner, 0.0f, 0.0f, 0.0f, DT_POSITION);
		obj->chState = RUNING;
		break;
	case RUNING:
...
fsm_rt_t motor_debug_state(fsm_cb_t *obj)
{
	...
	case ENTER:
		...
		foc_pid_init(&foc_param->velocity_pi_control, 0.2f, 0.020f, 10.0f); // 20251212 0.4f,20.5 目标速度-20~20
		kp = data[0];
		ki = data[1];
		foc_pid_init(&foc_param->pos_pi_control, 12.0f, 0.01f, 450.0f); // 20251212 10.5f 3.0f  目标位置0~24  响应时间400ms
#endif
#endif
		obj->chState = RUNING;
		break;
	...
	case RUNING:
	{
		...
		if (((obj->count) % POSITION_LOOP_DIVIDER == 0))
		{
			...
			if (vel_in < 2000.0f)
			{
				foc_param->velocity_ref = pos_loop_out + vel;
			}
			else
			{
				foc_param->velocity_ref = 2000.0f;
			}
		}
		if (((obj->count) % VELOCITY_LOOP_DIVIDER == 0))
		{
