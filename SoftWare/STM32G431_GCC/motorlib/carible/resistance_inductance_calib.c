    ...
    case MOTOR_PARAM_CALIB_STATE_MEASURE_L: {
        ...
        if (md->time_acc >= cfg->inductance_measure_duration) {
            ...
            if (md->iq_peak > 0.001f) {
                md->phase_inductance = md->vq_measure / 
...
{
    if (!motor_param_calib || !motor_param_calib->data) return;
    
    struct motor_param_calib_data *md = motor_param_calib->data;
    
    if (resistance) *resistance = md->phase_resistance;
    if (inductance) *inductance = md->phase_inductance;
}