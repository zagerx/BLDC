...
#define I2C_TRANSMIT_MODE      I2C_TRANSMIT_MODE_POLL
/* USER CODE END 0 */
...
void i2c2_read(uint16_t DevAddress, uint8_t register_addr, uint8_t *pData, uint16_t Size)
{
  ...
  uint8_t buf[1];
  buf[0] = register_addr;
  HAL_I2C_Master_Transmit(&hi2c2,DevAddress, buf,1, 0x10);
  HAL_I2C_Master_Receive(&hi2c2,DevAddress,&pData[0],2,0x10);
  ...
  HAL_I2C_Mem_Read_IT(&hi2c2,DevAddress,(uint16_t)(register_addr<<8),\
                      I2C_MEMADD_SIZE_16BIT,&i2cx_RxBuf[0],sizeof(i2cx_RxBuf));
#endif
}
void i2c2_write(uint16_t DevAddress, uint8_t register_addr,uint8_t *pData, uint16_t Size)
{
  ...
  SentTable[1] = (pData[0]);
  SentTable[2] = (pData[1]);
  HAL_I2C_Master_Transmit(&hi2c2, DevAddress, SentTable, sizeof(SentTable), 0x10);
}
/* USER CODE END 1 */
