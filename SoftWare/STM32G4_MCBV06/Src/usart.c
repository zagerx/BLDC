...
void MX_LPUART1_UART_Init(void)
{
  ...
  hlpuart1.AdvancedInit.TxPinLevelInvert = UART_ADVFEATURE_TXINV_DISABLE;
  hlpuart1.AdvancedInit.RxPinLevelInvert = UART_ADVFEATURE_TXINV_DISABLE;
  ...
  /* USER CODE BEGIN LPUART1_Init 2 */
   __HAL_UART_ENABLE_IT(&hlpuart1, UART_IT_IDLE);
    HAL_UART_Receive_DMA(&hlpuart1, (uint8_t*)sg_uartreceive_buff, sizeof(sg_uartreceive_buff));  
...
{
    if(LPUART1 == hlpuart1.Instance)                                   
    {
        if(RESET != __HAL_UART_GET_FLAG(&hlpuart1, UART_FLAG_IDLE))   
        {
            __HAL_UART_CLEAR_IDLEFLAG(&hlpuart1);                     
            HAL_UART_DMAStop(&hlpuart1);
            unsigned short data_length  = sizeof(sg_uartreceive_buff) - __HAL_DMA_GET_COUNTER(&hdma_lpuart1_rx);
